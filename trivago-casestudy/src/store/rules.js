...
export const survivalRule = (x, y, matrix, [isAlive, color = BLUE]) => {
	...
	if (isCellAlive(cell) && neighbours < 2) {
		return [false, color];
	}
	if (isCellAlive(cell) && (neighbours === 2 || neighbours === 3)) {
		return [true, color];
	}
	if (isCellAlive(cell) && neighbours > 3) {
		return [false, color];
	}
	if (!isCellAlive(cell) && neighbours === 3) {
		return [true, color];
...
export const getMostFrequentColor = neighbourColors => {
	let max = 1;
	let colorsFrequencyMap = {};
	let opColorCode;
	for (let i = 0; i < neighbourColors.length; i++) {
		const colorCode = neighbourColors[i];
		!opColorCode && (opColorCode = colorCode);
		if (colorsFrequencyMap[colorCode]) {
			++colorsFrequencyMap[colorCode];
			colorsFrequencyMap[colorCode] > max &&
				((max = colorsFrequencyMap[colorCode]),
				(opColorCode = colorCode));
		} else {
			colorsFrequencyMap[colorCode] = 1;
		}
	}
	return opColorCode;
};
